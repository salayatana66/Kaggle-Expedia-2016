...
def prepare_arrays_match():
    f = open("../feat/self-leaked-train-15-5-16.csv", "r")
    f.readline()
    ...
    best_hotels_country_market = dict()
    ...
    while 1:
        ...
        if hotel_market != '' and hotel_country != '':
            s4 = (hotel_market, hotel_country)
            if s4 in best_hotels_country_market:
                if hotel_cluster in best_hotels_country_market[s4]:
                    best_hotels_country_market[s4][hotel_cluster] += append_2
                else:
                    best_hotels_country_market[s4][hotel_cluster] = append_2
            else:
                best_hotels_country_market[s4] = dict()
                best_hotels_country_market[s4][hotel_cluster] = append_2
    ...
    f.close()
    return best_s00,best_s01, best_hotels_country, best_hotels_od_ulc, best_hotels_uid_miss, best_hotels_search_dest, popular_hotel_cluster, best_hotels_country_market


def gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster, best_hotels_country_market):
    now = datetime.datetime.now()
    path = '../subm/' + 'countmark-16-06-04' + '.csv'
    out = open(path, "w")
    ...
    while 1:
        ...
        s4 = (hotel_market, hotel_country)
        if s4 in best_hotels_country_market:
            d = best_hotels_country_market[s4]
            topitems = nlargest(5, d.items(), key=itemgetter(1))
            for i in range(len(topitems)):
                if topitems[i][0] in filled:
                    continue
...
best_s00,best_s01,best_hotels_country, best_hotels_od_ulc, best_hotels_uid_miss, best_hotels_search_dest, popular_hotel_cluster, best_hotels_country_market = prepare_arrays_match()
gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster, best_hotels_country_market)
